...
export default function AnalyticsChart({ productsList }: AnalyticsChartProps) {
    ...
    return (
        <Card id='analytics' className="bg-background border p-4">
            <div className="flex justify-between items-center mb-4">
                <div>
                    <h2 className="text-lg font-semibold text-white">Price and Rating Distribution</h2>
                    <p className="text-muted-foreground font-medium text-xl">Analytics </p>
                </div>
            </div>
            <CardContent className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={paginatedData} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                        ...
                        />
                        <Tooltip />
                        <Area type="monotone" dataKey="price" stroke="#4ade80" fillOpacity={1} fill="url(#colorPrice)" />
                        <Area type="monotone" dataKey="rating" stroke="#22d3ee" fillOpacity={1} fill="url(#colorRating)" />
                    </AreaChart>
                </ResponsiveContainer>
            </CardContent>
            ...
            <div className="mt-4 flex items-center justify-between px-2 text-sm text-muted-foreground">
                <div>{`Showing ${paginatedData.length} of ${products.length} products.`}</div>
